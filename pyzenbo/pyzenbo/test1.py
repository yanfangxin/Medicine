...
zenbo.robot.dynamic_edit_instance(
   "1207", DynamicEditAction.ADD_NEW_INSTANCE, "WhenHearWords", ['我問完了'])
zenbo.robot.set_listen_context(
   "1207", "stop_script,intercept_cross_intent,Context_WhenHear")
time.sleep(int(1))
...
for count in range(1000000):
    ...
    for i in range(lenth):
        if myUtterance == qa_data.iloc[i, 0]:  # 使用 iloc[i, 0] 來訪問第一列
            # 計算有效回答的數量（不包括 NaN）
            valid_responses = []
            for j in range(1, len(qa_data.columns)):  # 從第二列開始遍歷
                if not pd.isna(qa_data.iloc[i, j]):
                    valid_responses.append(qa_data.iloc[i, j])
            
            if valid_responses:  # 如果有有效回答
                # 隨機選擇一個回答
                response = random.choice(valid_responses)
                
                zenbo_speakLanguage = 1
                try:
                    zenbo.robot.set_expression(
                        RobotFace.PREVIOUS, 
                        response,  # 使用選中的回答
                        {
                            'speed': zenbo_speakSpeed, 
                            'pitch': zenbo_speakPitch, 
                            'languageId': zenbo_speakLanguage
                        },
                        sync=True
                    )
                    time.sleep(0.5)
                except Exception as e:
                    print(f"設置表情時發生錯誤：{str(e)}")
try:
    while True:
        time.sleep(int(10))
finally:
